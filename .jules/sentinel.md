...
## 2025-02-14 - Information Exposure via Unhandled Exception in API Response
**Vulnerability:** Raw `Exception` objects passed to the API response formatting function could cause the JSON serializer to crash or bypass standard security masking if the status code defaulted to 200, potentially leading to 500 Server Errors or exposing stack traces/internal details in logs or unhandled responses.
**Learning:** Defensive checks for `Exception` types must be evaluated independently of user-supplied status codes to ensure fail-secure behavior.
**Prevention:** Always prioritize exception type checking (`isinstance(data, Exception)`) before evaluating status codes, forcefully apply generic error messages, and automatically upgrade erroneous success status codes (e.g., 200) to server errors (e.g., 500) when an exception is detected.
