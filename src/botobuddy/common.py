...
def get_aws_session(session_config: dict = {}) -> boto3.Session:
    ...
    if assume_role := session_config.get("assume_role"):
        ...
        assumed_role_object = sts_client.assume_role(
            RoleArn=assume_role, RoleSessionName=session_name
        )
...
def get_aws_client(
    service, session_config={}, resource=False, core_config: dict | None = None
) -> AWSClient:
...
def get_cognito_client(session_config={}) -> CognitoIdentityProviderClient:
    return cast(
        CognitoIdentityProviderClient, get_aws_client("cognito-idp", session_config)
    )
